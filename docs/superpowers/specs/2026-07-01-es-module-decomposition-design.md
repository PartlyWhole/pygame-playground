...
| 19 | `src/main.mjs` | 110 | med | `init(pySeam)`: eager static imports of all modules; explicit init order (publish seams → populate `#examples` → `loadInitialProject` → renders); ALL cross-module injection; the four `editor.on('change')` registrations **in one place, in today's effective order** (armLint hook → autosave → promote-on-edit → collab bindEditor-on-join) — CORRECTED per as-built: the original text here was backwards, and the Task-10 review proved the order is NOT load-bearing; document it, don't defend it |
